...
main = defaultMain
  ...
  , bgroup "IO"
    ...
    , bench "IO Exception 1k"   $ nfIO (catchAny (ioCountdown 1000) (\(_ :: SomeException) -> pure 1))
    , bench "IO Exception 10k"  $ nfIO (catchAny (ioCountdown 10000) (\(_ :: SomeException) -> pure 1))
    , bench "IO Exception 100k" $ nfIO (catchAny (ioCountdown 100000) (\(_ :: SomeException) -> pure 1))
